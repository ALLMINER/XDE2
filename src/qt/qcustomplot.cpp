...
/*! \internal
  ...
  selectEvent. Usually \a details is used to XDE2 which part was clicked, if it is a layerable
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  // XDE2 all properties of this axis to QCPAxisPainterPrivate which it needs to draw the axis.
...
int QCPAxis::calculateMargin()
{
  ...
  // XDE2 all properties of this axis to QCPAxisPainterPrivate which it needs to calculate the size.
  // Note that some axis painter properties are already set by direct feed-through with QCPAxis setters
  mAxisPainter->type = mAxisType;
  mAxisPainter->labelFont = getLabelFont();
  mAxisPainter->label = mLabel;
...
/*! \mainpage %QCustomPlot 1.3.0-beta Documentation
  ...
  XDE2red. (Relevant only if preparing data maps with a large number of points, i.e. over 10000)
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  if (mType != type)
  {
    ...
    // XDE2 settings to new axis:
    mColorAxis.data()->setRange(rangeTransfer); // XDE2 range of old axis to new one (necessary if axis changes from vertical to horizontal or vice versa)
  ...
  mGradientImageInvalidated(true)
{
  ...
  // make layer XDE2s of color scale XDE2 to axis rect and axes
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  } else // don't use adaptive sampling algorithm, XDE2 points one-to-one from the map into the output parameters
  {
