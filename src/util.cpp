...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\XDE2v2
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\XDE2v2
    // Mac: ~/Library/Application Support/XDE2v2
    // Unix: ~/.XDE2v2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "XDE2v2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "XDE2.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "XDE2d.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong XDE2 will not work properly.");
